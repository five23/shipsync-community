...
class IllApps_Shipsync_Model_Shipping_Carrier_Fedex_Address extends IllApps_Shipsync_Model_Shipping_Carrier_Fedex
{
    ...
    {
	// Clean null streets
	foreach ($request->getStreet() as $street) { if ($street != '') { $streets[] = $street; }}
	...
	{
	    return array(Mage::helper('customer')->__($this->_addressResultError));
	}
	...
	$request['Version'] = array('ServiceId' => 'aval', 'Major' => $addressServiceVersion, 'Intermediate' => '0', 'Minor' => '0');
	...
	if (isset($response->AddressResults->ProposedAddressDetails->DeliveryPointValidation)
	       && $response->AddressResults->ProposedAddressDetails->DeliveryPointValidation == 'CONFIRMED')
	{
	    if ($response->AddressResults->ProposedAddressDetails->ResidentialStatus == 'RESIDENTIAL') { return true; }
	...
	$request['AddressesToValidate'] = array(0 => array(
	    'AddressId' => 'Destination',
	    'Address' => array(
	...
	elseif ($response->AddressResults->ProposedAddressDetails->DeliveryPointValidation == 'UNCONFIRMED' &&
                $response->AddressResults->ProposedAddressDetails->ResidentialStatus == 'INSUFFICIENT_DATA')
	{
	    if (is_array($response->AddressResults->ProposedAddressDetails->ParsedAddress->ParsedPostalCode->Elements))
	    {
		$postcode = $response->AddressResults->ProposedAddressDetails->ParsedAddress->ParsedPostalCode->Elements[0]->Value;
