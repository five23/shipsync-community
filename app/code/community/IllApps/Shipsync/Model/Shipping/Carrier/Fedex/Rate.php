...
class IllApps_Shipsync_Model_Shipping_Carrier_Fedex_Rate extends IllApps_Shipsync_Model_Shipping_Carrier_Fedex
{
    ...
    protected $_rateServiceVersion = '9';
    protected $_rateServiceWsdlPath = 'RateService_v9.wsdl';
	...
	$rateRequest = $this->_rateRequest;
        ...
        $rateRequest->setDefaultPackages(Mage::getModel('shipsync/shipping_package')->getDefaultPackages(array('fedex')));
	...
	$rateRequest->setValue(Mage::getModel('shipsync/shipping_package')->getPackageValue($rateRequest->getItems()));
        ...
        {            
            $rateRequest->setValueWithDiscount($rateRequest->getValue() -
                Mage::getModel('shipsync/shipping_package')->getPackageDiscount($rateRequest->getItems(), $rateRequest->getOrder()));
        }
	...
	$request['Version'] = array('ServiceId' => 'crs', 'Major' => $rateServiceVersion, 'Intermediate' => '0', 'Minor' => '0');
	...
	if ($rateRequest->getSaturdayDelivery())
	{
        ...
        {
	    $request['RequestedShipment']['ShippingChargesPayment'] = array ('PaymentType' => 'THIRD_PARTY', 'Payor' => array(
		'AccountNumber' => $rateRequest->getThirdPartyFedexAccount(),
		'CountryCode'   => $rateRequest->getThirdPartyFedexAccountCountry()));
        }
        else
        {
	    $request['RequestedShipment']['ShippingChargesPayment'] = array ('PaymentType' => 'SENDER', 'Payor' => array(
		'AccountNumber' => $this->getFedexAccount(),
	...
	{	    	    
	    ...
	    {
		$this->_rateResultError = $rateRequest->getShippingPackage()->getPackageError(); return $rateResult;
	    }
		...
		$i=0; foreach($packages as $package)
		{
		    ...
		    if ($weightUnit == 'G')
		    {
			$package['weight'] = $package['weight'] * 0.001; $weightUnit = 'KG';
		    }
                    ...
                    if ($rateRequest->getOrigCountry() != $rateRequest->getDestCountry())
                    {
                        ...
                        foreach ($itemsById as $qty => $item)
                        {
                            ...
                            $commodities[] = array(
                                    ...
                                    'Weight' => array('Units' => $weightUnit, 'Value' => $item['weight']),
                                    'Quantity' => $item['qty_to_ship'],
                                    'QuantityUnits' => 'EA',
                                    'UnitPrice' => array('Currency' => $this->getCurrencyCode(), 'Amount' => $itemValue),
                                    'CustomsValue' => array('Currency' => $this->getCurrencyCode(), 'Amount' => ($itemValue * $item['qty_to_ship']))
                                );
                        }
                    }

                    if (isset($commodities)) {$request['RequestedShipment']['CustomsClearanceDetail']['Commodities'] = $commodities; }
                    ...
                    else if (Mage::getStoreConfig('carriers/fedex/rating_insured_value'))
		    {
                        if (isset($package['package_value']))
                        {
                            ...
                            $package_value = 0.0;
                            foreach ($package['items'] as $item) { $package_value += $item['value']; }
                        }
	    ...
	    else { $this->_rateResultError = 'Unable to estimate packages'; return $rateResult; }
	}
	else { $this->_rateResultError = 'No items found to ship'; return $rateResult; }
	...
	$rateResult  = $this->_rateResult;
	   ...
	   ($response->HighestSeverity == 'WARNING' && !isset($response->RateReplyDetails)))
	    ...
	    $this->_rateResultError = $errorMsg; return $rateResult;
        }
	elseif (!isset($response->RateReplyDetails))
	{
	    $this->_rateResultError = 'Error: Empty rate result.  Please contact your system administrator.'; return $rateResult;
	}
        ...
        foreach ($rateReplyDetails as $rateReply)
	{
		...
		if (Mage::getStoreConfig('carriers/fedex/free_shipping_enable') && $continentalTest)
		{
			...
			$value = $this->_rateRequest->getValueWithDiscount();
                    ...
                    $freeMethods = array(Mage::getStoreConfig('carriers/fedex/free_method'));
		    ...
		    if (in_array($rateResultMethod->getMethod(), $freeMethods) &&
                        $value > Mage::getStoreConfig('carriers/fedex/free_shipping_subtotal'))
		    {
	...
	if (!isset($rateResult)) { $this->_rateResultError('No applicable rates available'); return $rateResult; }
